...
export class HomeService {
  ...
  private readonly HOME_DAYS_EVENTS_URL = this.HOME_URL + '/dailyEvents';
  ...
  // Daily Events  
  public getDaysEvents(): Observable<{ data: DailyEvents[] }> {
    return this.http.get<{ data: DailyEvents[] }>(this.HOME_DAYS_EVENTS_URL);
  }

  public postDaysEvents(data): Observable<{ message: string }> {
    return this.http.post<{ message: string }>(
      this.HOME_DAYS_EVENTS_URL,
      data
    );
  }
}
